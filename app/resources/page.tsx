...
// ScrollReveal component for better scroll animations
...
export default function ResourcesPage() {
  ...
  const serviceCategories = [
    { name: "Website Maintenance", icon: Shield, color: "bg-blue-100 text-blue-600" },
    { name: "Website Copywriting", icon: PenTool, color: "bg-purple-100 text-purple-600" },
    { name: "B2B Marketing", icon: Megaphone, color: "bg-orange-100 text-orange-600" },
    { name: "Webflow Agency", icon: Globe, color: "bg-green-100 text-green-600" },
    { name: "B2B Branding", icon: Briefcase, color: "bg-pink-100 text-pink-600" }
  ];

  // Row component for horizontal scrolling with direction control
function ScrollingRow({
  ...
  return (
    <div className="relative mb-6 overflow-hidden">
      <div ref={scrollRef} className="flex gap-3 whitespace-nowrap">
        {tripledCategories.map((cat, idx) => (
          <motion.button
            key={`${rowIndex}-${idx}`}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="px-6 py-3 rounded-full text-sm font-medium bg-slate-100"
          >
            {cat}
          </motion.button>
        ))}
  ...
  return (
    <main className="bg-white overflow-x-hidden">
      {/* Hero Section - Centered with new text */}
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-50 via-white to-orange-50/30 px-8 pt-24 pb-20">
      ...
      {/* Explore by topics - Horizontal Scrolling Section */}
      <section className="px-8 py-16 max-w-7xl mx-auto">
        <ScrollReveal>
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-black tracking-tighter mb-4">Explore by topics</h2>
            <p className="text-slate-600 max-w-2xl mx-auto">Find exactly what you need from our curated collection of B2B resources</p>
          </div>
        </ScrollReveal>
      ...
      {/* The Blog Section with Sidebar */}
      <section className="px-8 py-20 max-w-7xl mx-auto">
        <ScrollReveal>
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-black tracking-tighter mb-4">The Blog</h2>
            <p className="text-slate-600 max-w-2xl mx-auto">Insights from experts on remote work, web design, founder journeys, and marketing tactics</p>
          </div>
        </ScrollReveal>
        ...
        <div className="flex flex-col lg:flex-row gap-12">
                ...
                >
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3">
                      <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-black text-white ${
                  ...
                  <div className="p-6">
                    ...
                    <h3 className="text-xl font-black tracking-tight mb-3 group-hover:text-deep-orange transition-colors line-clamp-2">
                      {post.title}
                    </h3>
                    <p className="text-slate-500 text-sm mb-4 line-clamp-2">{post.excerpt}</p>
                    <Link href="#" className="inline-flex items-center gap-2 font-black text-sm text-deep-orange hover:gap-4 transition-all">
          ...
          <div className="lg:w-80 flex-shrink-0">
            ...
            <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 mb-8 text-white">
              <div className="bg-deep-orange w-12 h-12 rounded-xl flex items-center justify-center mb-4">
                <Mail size={24} />
              </div>
              <h3 className="text-xl font-black mb-2">Subscribe to the Newsletter</h3>
              <p className="text-slate-300 text-sm mb-4">Get weekly insights delivered to your inbox. No spam, ever.</p>
              <input 
                type="email" 
                placeholder="Your email address" 
                className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-white placeholder:text-slate-400 mb-3 focus:outline-none focus:ring-2 focus:ring-deep-orange"
              />
              <button className="w-full bg-deep-orange text-white py-3 rounded-xl font-black hover:bg-orange-600 transition-all">
                Subscribe Now
              </button>
            </div>

            {/* Marketing Audit CTA */}
            <div className="bg-gradient-to-br from-deep-orange to-orange-600 rounded-2xl p-6 text-white">
              <div className="bg-white/20 w-12 h-12 rounded-xl flex items-center justify-center mb-4">
                <Target size={24} />
              </div>
              <h3 className="text-xl font-black mb-2">Get a Marketing Audit</h3>
              <p className="text-orange-100 text-sm mb-4">Get a free, no-obligation audit of your B2B website's conversion potential.</p>
              <Link href="/talk-to-us">
                <button className="w-full bg-white text-deep-orange py-3 rounded-xl font-black hover:bg-slate-100 transition-all">
            ...
            <div className="bg-slate-50 rounded-2xl p-6 mt-8">
              <h3 className="text-lg font-black mb-4 flex items-center gap-2"><Tag size={18} /> Categories</h3>
              <div className="space-y-3">
                {blogCategories.map((cat, idx) => (
                  <Link key={idx} href="#" className="flex items-center justify-between group">
                    <div className="flex items-center gap-3">
                      <div className={`${cat.color} w-8 h-8 rounded-lg flex items-center justify-center`}>
                        <cat.icon size={16} />
                      </div>
                      <span className="text-slate-700 group-hover:text-deep-orange transition-colors">{cat.name}</span>
                    </div>
      ...
      {/* Careers Section - High Energy Recruitment Page (Without Current Openings) */}
      <section className="px-8 py-20 bg-gradient-to-r from-slate-900 to-slate-800 my-16">
        <div className="max-w-7xl mx-auto">
          <ScrollReveal>
            <div className="text-center mb-12">
              <div className="inline-flex items-center gap-2 bg-deep-orange/20 px-4 py-2 rounded-full mb-4">
                <Award size={16} className="text-deep-orange" />
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {[
              ...
              >
                <item.icon size={32} className="text-deep-orange mx-auto mb-3" />
          ...
          {/* Bottom CTA - View Openings Button */}
          <div className="text-center">
            <Link href="/careers">
              <motion.button 
                whileHover={{ scale: 1.05 }}
                className="bg-deep-orange text-white px-10 py-4 rounded-2xl font-black inline-flex items-center gap-2"
              >
                View Open Positions <ArrowRight size={18} />
              </motion.button>
            </Link>
            <p className="text-slate-400 text-sm mt-4">Join a team of passionate builders working with global clients</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-8">
        <ScrollReveal>
          <div className="max-w-7xl mx-auto bg-slate-900 rounded-[4rem] p-16 md:p-20 flex flex-col lg:flex-row items-center gap-16 text-white">
            <div className="flex-1">
              <motion.div 
                whileHover={{ rotate: 5, scale: 1.05 }}
                className="bg-deep-orange w-16 h-16 rounded-2xl flex items-center justify-center mb-8"
              >
              ...
              <div className="flex flex-col gap-4">
                <Link href="/talk-to-us" className="w-full">
                  <motion.button 
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="bg-deep-orange text-white w-full px-8 py-5 rounded-2xl font-black hover:bg-orange-600 transition-all"
                  >
                    Contact Us
                  </motion.button>
                </Link>
      ...
      {/* Final CTA */}
      <section className="py-32 px-8 text-center bg-slate-50">
        <ScrollReveal>
          <div className="max-w-4xl mx-auto">
            <motion.div 
              whileHover={{ scale: 1.05 }}
              className="inline-flex items-center gap-2 bg-deep-orange/10 px-4 py-2 rounded-full mb-8"
            >
              <Zap size={16} className="text-deep-orange" />
            ...
            <Link href="/talk-to-us">
              <motion.button 
                ...
                whileTap={{ scale: 0.95 }}
                className="bg-black text-white px-12 py-5 rounded-2xl font-black text-lg hover:bg-slate-800 transition-all inline-flex items-center gap-2"
              >
